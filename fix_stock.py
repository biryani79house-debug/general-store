...
print("Updating product stock values to match purchases - sales...")
print("=" * 60)
...
for product in products:
    ...
    print(f"Updated {product.name}: {old_stock:.2f} → {calculated_stock:.2f} {product.unit_type} (change: {difference:+.2f})")

db.commit()
print("\nStock values updated successfully!")

# Verify the updates
print("\nVerification - Updated stock values:")
products = db.query(Product).all()
for product in products:
    print(f"{product.name}: {product.stock} {product.unit_type}")
